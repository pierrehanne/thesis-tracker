...
class DataProcessor:
    ...
    def format_theses_data(data: pd.DataFrame) -> pd.DataFrame:
        ...
        languages = set(
            lang["langue"] for subjects in data["subjects"] for lang in subjects
        )
        for lang in languages:
            data[f"topic_{lang}"] = data["subjects"].apply(
                lambda subjects: ", ".join(
        ...
        data = data.fillna("")
        data["date_submission"] = pd.to_datetime(
            data["date_submission"], errors="coerce"
        )
